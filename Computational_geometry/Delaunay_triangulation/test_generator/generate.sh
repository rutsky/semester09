...
if true; then
  ...
  for n in `seq 3 99`; do
    for i in `seq 1 5`; do
      python lattice_uniform.py $n 0 7 0 7 > $TESTS_DIR/lattice_uniform_8x8_`printf '%03d' $n`_$i.in
    done
  done
  for n in `seq 3 99`; do
    for i in `seq 1 5`; do
      python lattice_uniform.py $n 0 19 0 2 > $TESTS_DIR/lattice_uniform_3x20_`printf '%03d' $n`_$i.in
    done
  done
fi
