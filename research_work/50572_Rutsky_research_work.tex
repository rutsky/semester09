...
Высота съёмки над уровнем Земли в метрах: $C_1.h, C_2.h \in [800, 1000]$.
...
Для камер заданы пирамиды видомости $C_1.V$ и $C_2.V$: 
даны направления камер $C_1.\bar{d}$, $C_2.\bar{d}$ и углы зрения $C_1.\alpha$, $C_2.\gamma$.
Направления съёмки близки к вертикальным вниз: $\widehat{(C_i.\bar{d}, \ -\bar{n}\ )} < 30^{\circ}$,
а углы зрения $C_i.\alpha$, $C_i.\gamma \in [30^{\circ}, 60^{\circ}]\quad i=1,2$.
Дано взаимное расположение камер $\bar{s}=C_1.\bar{p} - C_2.\bar{p}$, 
где $C_i.\bar{p}\quad i=1,2$~--- вектора положений камер.

Пусть $G_i = C_i.V \cap P$~--- часть плоскоти $P$, видимая из камеры $C_i\quad i=1,2$.
